...
def render_general_settings():
    ...
    with col3:
        default_priority = st.selectbox("Default Priority", ["Low", "Medium", "High"], index=1)
        st.session_state.default_priority = default_priority
    
    with col4:
        default_category = st.selectbox("Default Category", config.TASK_CATEGORIES, index=0)
        st.session_state.default_category = default_category
    
    # Time zone
    timezone = st.selectbox("Timezone", ["UTC", "US/Eastern", "US/Central", "US/Mountain", "US/Pacific"], index=0)
...
def render_notification_settings():
    ...
    if desktop_enabled:
        ...
        with col2:
            notification_timeout = st.slider("Notification timeout (seconds)", 5, 30, 10)
    ...
    if email_enabled:
        ...
        if daily_summary:
            summary_time = st.time_input("Daily summary time", value=datetime.strptime("09:00", "%H:%M").time())
...
def render_email_settings():
    ...
    st.warning("⚠️ Email configuration requires editing the .env file. The settings below are for reference only.")
...
def clear_completed_tasks():
    ...
    st.session_state.tasks = [task for task in st.session_state.tasks if not task.get('completed', False)]
